...
\long\def\pgfcircdeclarequadpole#1#2#3{
    \pgfdeclareshape{#1}
    {
        ...
        \anchor{text}{\pgfextractx{\pgf@xa}{\textoffset}\pgfextracty{\pgf@ya}{\textoffset}
            \northwest
            \pgf@x=\pgf@xa \advance\pgf@y by \pgf@ya
        }
        #3%
...
% Switch the plot-type filter to ideal shapes
\newif\ifpgf@circuit@idealfilter
\ctikzset{ideal filter/.is if=pgf@circuit@idealfilter}%
\tikzset{ideal filter/.is if=pgf@circuit@idealfilter}%
% Add small "0" and "1" to ideal filters plots
\newif\ifpgf@circuit@idealfilter@numbers
\ctikzset{ideal filter numbers/.is if=pgf@circuit@idealfilter@numbers}%
\tikzset{ideal filter numbers/.is if=pgf@circuit@idealfilter@numbers}%
\ctikzset{ideal filter number low/.initial={0}}
\ctikzset{ideal filter number high/.initial={1}}
% set up plot-type drawing
\def\pgfcirc@setup@plot@filters{%
    \ifpgf@circuit@idealfilter
        \pgfcirc@twoport@draw@linax
    \else
        \pgfcirc@twoport@draw@logax
    \fi
    \edef\@@tmp{\ctikzvalof{blocks/filter plot/relative thickness}}
    \pgfsetlinewidth{\@@tmp\pgflinewidth}
    % ideal filters are "square"
    \ifpgf@circuit@idealfilter\else
        \edef\@@tmp{\ctikzvalof{blocks/filter plot/rounding}}
        \pgfsetcornersarced{\pgfpoint{\@@tmp\pgf@circ@res@step}{\@@tmp\pgf@circ@res@step}}
    \fi
}
% draw a fake lin-lin grid for ideal plot-type filters
\def\pgfcirc@twoport@draw@linax{%
    \pgfscope
        ...
        \ifdim\@@opacity pt>0pt
            ...
            \def\@@gridx{0.7cm}\def\@@gridy{0.6cm}
            % horizontal lin
            \pgfpathmoveto{\pgfpoint{-\@@gridx}{-\@@gridy}}
            \pgfpathlineto{\pgfpoint{\@@gridx}{-\@@gridy}}
            \pgfpathmoveto{\pgfpoint{-\@@gridx}{-0.5*\@@gridy}}
            \pgfpathlineto{\pgfpoint{\@@gridx}{-0.5*\@@gridy}}
            \pgfpathmoveto{\pgfpoint{-\@@gridx}{0cm}}
            \pgfpathlineto{\pgfpoint{\@@gridx}{0cm}}
            \pgfpathmoveto{\pgfpoint{-\@@gridx}{0.5*\@@gridy}}
            \pgfpathlineto{\pgfpoint{\@@gridx}{0.5*\@@gridy}}
            \pgfpathmoveto{\pgfpoint{-\@@gridx}{\@@gridy}}
            \pgfpathlineto{\pgfpoint{\@@gridx}{\@@gridy}}
            ...
            \pgfpathmoveto{\pgfpoint{-\@@gridx}{-\@@gridy}}
            \pgfpathlineto{\pgfpoint{-\@@gridx}{\@@gridy}}
            \pgfpathmoveto{\pgfpoint{-0.5*\@@gridx}{-\@@gridy}}
            \pgfpathlineto{\pgfpoint{-0.5*\@@gridx}{\@@gridy}}
            \pgfpathmoveto{\pgfpoint{0cm}{-\@@gridy}}
            \pgfpathlineto{\pgfpoint{0cm}{\@@gridy}}
            \pgfpathmoveto{\pgfpoint{0.5*\@@gridx}{-\@@gridy}}
            \pgfpathlineto{\pgfpoint{0.5*\@@gridx}{\@@gridy}}
            \pgfpathmoveto{\pgfpoint{\@@gridx}{-\@@gridy}}
            \pgfpathlineto{\pgfpoint{\@@gridx}{\@@gridy}}
            ...
            \pgfusepath{draw}
            \ifpgf@circuit@idealfilter@numbers
                \pgftext[right,at={\pgfpoint{-\@@gridx}{-\@@gridy}}]{%
                    \scriptsize\ctikzvalof{ideal filter number low}\kern1pt}
                \pgftext[right,at={\pgfpoint{-\@@gridx}{\@@gridy}}]{%
                    \scriptsize\ctikzvalof{ideal filter number high}\kern1pt}
            \fi
        \fi
    \endpgfscope
}
% draw a fake log-dB(lin) grid for plot-style filters
...
{\ctikzvalof{bipoles/lowpassp/width}}
{
    ...
    \pgfpathlineto{\pgfpoint{0pt}{0.6\pgf@circ@res@step}}
    \ifpgf@circuit@idealfilter
        \pgfpathlineto{\pgfpoint{ 0pt}{-0.6\pgf@circ@res@step}}
...
{\ctikzvalof{bipoles/bandpassp/width}}
{
    ...
    \pgfpathmoveto{\pgfpoint{-0.7\pgf@circ@res@step}{-0.6\pgf@circ@res@step}}
    \ifpgf@circuit@idealfilter
        \pgfpathlineto{\pgfpoint{-0.35\pgf@circ@res@step}{-0.6\pgf@circ@res@step}}
        \pgfpathlineto{\pgfpoint{-0.35\pgf@circ@res@step}{0.6\pgf@circ@res@step}}
        \pgfpathlineto{\pgfpoint{0.35\pgf@circ@res@step}{0.6\pgf@circ@res@step}}
        \pgfpathlineto{\pgfpoint{0.35\pgf@circ@res@step}{-0.6\pgf@circ@res@step}}
    \else
...
{\ctikzvalof{bipoles/resonantp/width}}
{
    ...
    \else
        ...
        \pgfpathlineto{\pgfpoint{0pt}{0.7\pgf@circ@res@step}}
        \pgfpathlineto{\pgfpoint{ 0.15\pgf@circ@res@step}{-0.6\pgf@circ@res@step}}
    \fi
    \pgfpathlineto{\pgfpoint{ 0.7\pgf@circ@res@step}{-0.6\pgf@circ@res@step}}
    \pgfusepath{draw}
}
%% notch plot-type filter
\pgfcirc@define@twoports{blocks}
{}
...
%
\def\pgcirc@twoport@additional@northwest@anchors{%
    \savedmacro{\blockleftanchorpos}{\edef\blockleftanchorpos{\ctikzvalof{block left anchors pos}}}
    \savedmacro{\blockrightanchorpos}{\edef\blockrightanchorpos{\ctikzvalof{block right anchors pos}}}
    \anchor{left down}{\northwest\pgf@y=-\blockleftanchorpos\pgf@y}
    \anchor{right down}{\northwest\pgf@x=-\pgf@x\pgf@y=-\blockrightanchorpos\pgf@y}
    \anchor{right up}{\northwest\pgf@x=-\pgf@x\pgf@y=\blockrightanchorpos\pgf@y}
    \anchor{left up}{\northwest\pgf@y=\blockleftanchorpos\pgf@y}
    \anchor{up}{\northwest\pgf@x=0pt\relax}
    \anchor{down}{\northwest\pgf@y=-\pgf@y\pgf@x=0pt\relax}
}
...
\def\pgf@circ@circular@rf@anchors#1{%
    ...
    \pgcirc@twoport@additional@northwest@anchors
...
\pgfdeclareshape{oscillator}
{
    ...
    % text anchor, in the south-east corner
    \savedanchor{\textheight}{%
        \pgf@y= \ht\pgfnodeparttextbox %
    }
    \anchor{text}{\pgfextracty{\pgf@ya}{\textheight}
        \northwest \pgf@x=-1.1\pgf@x \pgf@y=-\pgf@y
        \advance\pgf@y by -0.1\pgf@ya
    }
    \pgf@circ@draw@component{
        \pgf@circ@setcolor
        \pgf@circ@scaled@Rlen=\scaledRlen
...
%% gridnode
\pgfkeys{tikz/gridlines/.code={\ctikzset{blocks/gridnode/gridlines=#1}}}
\pgfkeys{tikz/gridlines/.default=\ctikzvalof{blocks/gridnode/gridlines}}
\ctikzset{blocks/gridnode/gridlines/.initial=7}

\pgfdeclareshape{gridnode}
{
    \savedmacro{\ctikzclass}{\edef\ctikzclass{blocks}}
    \saveddimen{\scaledRlen}{\pgfmathsetlength{\pgf@x}{\ctikzvalof{\ctikzclass/scale}\pgf@circ@Rlen}}
    \savedmacro{\gridlines}{\edef\gridlines{\ctikzvalof{blocks/gridnode/gridlines}}}
    \savedanchor\northwest{
        \pgfmathsetlength{\pgf@circ@scaled@Rlen}{\ctikzvalof{\ctikzclass/scale}\pgf@circ@Rlen}
    ...
    \pgf@circ@draw@component{
        ...
        % draw grid
        \ifodd\gridlines
        	\edef\gridmiddleline{\numexpr(\gridlines+1)/2\relax}
        \else
        	\edef\gridmiddleline{\numexpr\gridlines/2+1\relax}
       	\fi
        \foreach \line in {\gridmiddleline,...,\gridlines}
       	{
            \pgfmathsetmacro\relpos{4*\line/(\gridlines+1) - 3}
            \pgfpathmoveto{\pgfpoint{\relpos\pgf@circ@res@right}{\pgf@circ@res@up}}
            \pgfpathlineto{\pgfpoint{\pgf@circ@res@right}{\relpos\pgf@circ@res@up}}
            \pgfpathmoveto{\pgfpoint{\relpos\pgf@circ@res@left}{\pgf@circ@res@up}}
            \pgfpathlineto{\pgfpoint{\pgf@circ@res@left}{\relpos\pgf@circ@res@up}}
        }
        %prevent from draw the inner cross twice
        \foreach \line in {1,...,\numexpr\gridmiddleline-1\relax}
        {
            \pgfmathsetmacro\relpos{1 - 4*\line/(\gridlines+1)}
            \pgfpathmoveto{\pgfpoint{\relpos\pgf@circ@res@right}{\pgf@circ@res@down}}
            \pgfpathlineto{\pgfpoint{\pgf@circ@res@right}{\relpos\pgf@circ@res@down}}
            \pgfpathmoveto{\pgfpoint{\relpos\pgf@circ@res@left}{\pgf@circ@res@down}}
            \pgfpathlineto{\pgfpoint{\pgf@circ@res@left}{\relpos\pgf@circ@res@down}}
      	}
        \pgfusepath{draw}
    }
}
...
\pgfdeclareshape{wilkinson}{
    ...
    \anchor{out2}{\northwest\pgf@x=-\pgf@x\pgf@y=0.5\pgf@y}
    \savedanchor{\textoffset}{%
        \pgf@y =  0.5\ht\pgfnodeparttextbox % (add)
        \pgf@x = -0.5\wd\pgfnodeparttextbox % (set)
    }
    \anchor{text}{\pgfextractx{\pgf@xa}{\textoffset}\pgfextracty{\pgf@ya}{\textoffset}%
        \northwest
        \pgf@x = \pgf@xa \advance \pgf@y by \pgf@ya
...
\pgfdeclareshape{genericsplitter}{
    ...
    \savedanchor{\textanchor}{%
        ...
        \pgf@x = -.2\wd\pgfnodeparttextbox % (set)
    }
