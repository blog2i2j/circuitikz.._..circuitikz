...
% FDSOI bulk connection symbol https://github.com/circuitikz/circuitikz/issues/871
\newif\ifpgf@circuit@trans@fdsoi
\pgf@circuit@trans@fdsoifalse
\ctikzset{transistors/fdsoi/.is if=pgf@circuit@trans@fdsoi}
\tikzset{fdsoi/.is if=pgf@circuit@trans@fdsoi}
% in simplified fet, use a dot symbol for back gate
\newif\ifpgf@circuit@trans@fdsoidot
\pgf@circuit@trans@fdsoidotfalse
\ctikzset{transistors/fdsoidot/.is if=pgf@circuit@trans@fdsoidot}
\ctikzset{transistors/fdsoidot/.is choice}
\ctikzset{transistores/fdsoidot/true/.code={\pgf@circuit@trans@fdsoitrue\pgf@circuit@trans@fdsoidottrue}}
\ctikzset{transistores/fdsoidot/false/.code={\pgf@circuit@trans@fdsoifalse\pgf@circuit@trans@fdsoidotfalse}}
\tikzset{fdsoidot/.code={\pgf@circuit@trans@fdsoitrue\pgf@circuit@trans@fdsoidottrue}}
\tikzset{nofdsoidot/.code={\pgf@circuit@trans@fdsoifalse\pgf@circuit@trans@fdsoidotfalse}}
...
\long\def\pgfcircdeclaretransistor#1#2#3{
    \pgfdeclareshape{#1}
    {
        ...
        \savedanchor{\textanchor}{
            ...
            \pgf@y=.5\pgf@y
            \pgf@x=0pt
            % extrabodydiodelen computed here
            \ifpgf@circuit@transisors@fixlabels
                % \typeout{DEBUG: Saving base text anchor for transistors, with fix}
                \pgf@circ@res@other=0pt\relax
                \ifpgf@circuit@fet@bodydiode
                    % try to put the text to the right of the flyback diode
                    \pgfmathsetlength{\pgf@circ@res@other}{(
                ...
                \fi
                \ifpgf@circ@trcircle
                    % in this case, the  offset that wins is the circle one
                    % left
                    \pgf@xa=-\ctikzvalof{tripoles/#1/width}\pgf@circ@scaled@Rlen
                    \pgf@xa=\circlebase\pgf@xa % this is the base point of the circle
                    % northeast
                    ...
                    %
                    \pgf@yb=\ctikzvalof{tripoles/#1/bodydiode conn}\pgf@yb %this is #3 of the circle
                    % find the position of the text as 2*\circleradius+ \circleleft
                    \pgfmathsetlength{\pgf@circ@res@other}{2*((\pgf@xb)-(\pgf@xa)+(\pgf@yb)*(\pgf@yb)/((\pgf@xb)-(\pgf@xa)))/2*\scalecircleradius + \pgf@xa}
                \fi
                \pgf@x=\pgf@circ@res@other
                \advance\pgf@x by 0.05\pgf@circ@scaled@Rlen\relax
                \ifpgf@circ@center@text
                    \pgf@y=0pt
                \else % if the text is not on the center line, can be up or down
                    \ifpgf@circ@trcircle
                        \pgf@y = 0.7\pgf@y
                        \advance \pgf@y by 0.3\pgf@x
                        \pgf@x = 0.7\pgf@x
                    \else
                        \pgf@y=0.7\pgf@y
                    \fi
                    \ifpgf@circ@up@text\else\pgf@y=-\pgf@y\fi
                \fi
                \advance\pgf@y by \dimexpr.5\dp\pgfnodeparttextbox-.5\ht\pgfnodeparttextbox\relax
            \else % fixlabels; legacy transistor text here
                % \typeout{DEBUG: Saving base text anchor for transistors, without fix}
                \pgf@y=.7\pgf@y
...
\long\def\declaregfet#1{
    \pgfcircdeclaretransistor{#1}{
        ...
        \savedanchor{\textanchor}{
            % need a different text: no circle, shift to the right
            %% \northeast
            \pgfmathsetlength{\pgf@circ@scaled@Rlen}{\ctikzvalof{\ctikzclass/scale}\pgf@circ@Rlen}
            \pgf@y=\ctikzvalof{tripoles/#1/height}\pgf@circ@scaled@Rlen
            \pgf@y=.5\pgf@y
            \pgf@x=0pt
            %% \gfettextoffset calculation here
            \ifpgf@circuit@fet@bodydiode
                % try to put the text to the right of the flyback diode
                \pgfmathsetlength{\pgf@circ@res@other}{(
                    \ctikzvalof{tripoles/#1/bodydiode distance}*
            ...
            \else
                \pgfmathsetlength{\pgf@circ@res@other}{\ctikzvalof{tripoles/#1/base width}*
                    \ctikzvalof{tripoles/#1/width}*\pgf@circ@scaled@Rlen}
            \fi
            \ifpgf@circuit@transisors@fixlabels
...
\def\pgfcirc@simplemos@drawbulk#1{%
    \ifpgf@circuit@bpt@drawbulk % added by Burak Kelleci
        \ifpgf@circuit@trans@fdsoi % FDSOI added by Romano Giannetti
            \pgfpathmoveto{\pgfpoint{0.4*\ctikzvalof{tripoles/#1/base width}*\pgf@circ@res@left}
            {\pgf@circ@res@up+\pgf@circ@res@down}}
        \else
            \pgfpathmoveto{\pgfpoint{\ctikzvalof{tripoles/#1/base width}*\pgf@circ@res@left}
            {\pgf@circ@res@up+\pgf@circ@res@down}}
        \fi
        \pgfpathlineto{\pgfpoint{\pgf@circ@res@right+\pgfhorizontaltransformationadjustment*.5*\pgflinewidth}
        {\pgf@circ@res@up+\pgf@circ@res@down}}
        \pgfusepath{draw}
        % back gate, with the same thickness than the main gate
        \ifpgf@circuit@trans@fdsoi
            \ifpgf@circuit@trans@fdsoidot
                \pgfpathcircle{\pgfpoint
                    {0.4*\ctikzvalof{tripoles/#1/base width}\pgf@circ@res@left}
                    {\pgf@circ@res@up+\pgf@circ@res@down}}{\ctikzvalof{nodes width}*\pgf@circ@Rlen}
                \pgfusepath{draw,fill}
            \else
                \pgfscope
                    \pgfpathmoveto{\pgfpoint
                    {0.4*\ctikzvalof{tripoles/#1/base width}*\pgf@circ@res@left}{1.2*\pgf@circ@res@up+\pgf@circ@res@down}}
                    \pgfpathlineto{\pgfpoint
                    {0.4*\ctikzvalof{tripoles/#1/base width}*\pgf@circ@res@left}{\pgf@circ@res@up+1.2*\pgf@circ@res@down}}
                    \pgf@circ@setlinewidth{tripoles}{\pgflinewidth}
                    \pgfusepath{draw}
                \endpgfscope
            \fi
...
\pgfcircdeclaretransistor{nmos}{
    ...
    \pgfcirc@simplemos@drawbulk{nmos}
...
\pgfcircdeclaretransistor{pmos}{
    ...
    \pgfcirc@simplemos@drawbulk{pmos}
...
\pgfcircdeclaretransistor{nmosd}{
    ...
    \pgfcirc@simplemos@drawbulk{nmosd}
...
\pgfcircdeclaretransistor{pmosd}{
    ...
    \pgfcirc@simplemos@drawbulk{pmosd}
...
\long\def\drawfetcore#1{
    ...
    \ifpgf@circuit@trans@fdsoi
        \pgfpathmoveto{\pgfpoint
        {\ctikzvalof{tripoles/#1/base width}*\pgf@circ@res@left}{\pgf@circ@res@up+\pgf@circ@res@down}}
        % beware, fixed numbers here. The 0.5 MUST coincide with @@final below in arrow positioning
        \pgfpathlineto{\pgfpoint
        {0.5*\ctikzvalof{tripoles/#1/base width}*\pgf@circ@res@left}{\pgf@circ@res@up+\pgf@circ@res@down}}
        \pgfpathmoveto{\pgfpoint
        {0.3*\ctikzvalof{tripoles/#1/base width}*\pgf@circ@res@left}{\pgf@circ@res@up+\pgf@circ@res@down}}
    \else
        \pgfpathmoveto{\pgfpoint
        {\ctikzvalof{tripoles/#1/base width}\pgf@circ@res@left}{\pgf@circ@res@up+\pgf@circ@res@down}}
    \fi
    ...
    \ifpgf@circuit@trans@fdsoi
        \pgfscope
            \pgfpathmoveto{\pgfpoint
            {0.3*\ctikzvalof{tripoles/#1/base width}*\pgf@circ@res@left}{1.2*\pgf@circ@res@up+\pgf@circ@res@down}}
            \pgfpathlineto{\pgfpoint
            {0.3*\ctikzvalof{tripoles/#1/base width}*\pgf@circ@res@left}{\pgf@circ@res@up+1.2*\pgf@circ@res@down}}
            \pgf@circ@setlinewidth{tripoles}{\pgflinewidth}
            \pgfusepath{draw}
        \endpgfscope
    \fi
    ...
    \pgfscope
        ...
        \pgfallowupsidedownattimetrue
        \pgfresetnontranslationattimefalse
        % this is an OR, see https://tex.stackexchange.com/a/29906/38080
        % I love TeX...
        \edef\@@final{1.0} %position in the bulk line
        \ifnum\ifpgf@circuit@trans@arrowatend 1\else\ifpgf@circuit@trans@fdsoi 1\else 0 \fi\fi =1 %
                \ifpgf@circuit@trans@ntype
                    \edef\@@anchor{tip}
                    \edef\@@pos{1.0}
                \else
                    ...
                    \ifpgf@circuit@trans@fdsoi
                        \edef\@@pos{1.0}
                        \edef\@@final{0.5}% this MUST coincide with the bulk drawing above
                    \fi
                \fi
        \else
        ...
        \pgftransformlineattime{\@@pos}{%
            ...
            \pgfpoint%
            {\@@final*\ctikzvalof{tripoles/#1/base width}*\pgf@circ@res@left}%
            {\pgf@circ@res@up+\pgf@circ@res@down}%
        }
        \ifpgf@circuit@trans@ntype
        \else
...
%% Amplifiers should be rewritten from scratch to factor out common things.
%% Better, if there where a anchor alias facility, rewrite them in terms of muxdemux-es
%%
%% Macro support for border anchors
\def\pgf@circ@amps@leftedge#1{%
    \pgfmathsetlength{\pgf@circ@scaled@Rlen}{\ctikzvalof{\ctikzclass/scale}\pgf@circ@Rlen}
    \pgf@x=-\ctikzvalof{tripoles/#1/width}\pgf@circ@scaled@Rlen
    \pgf@x=.5\pgf@x
    \pgf@x = \ctikzvalof{tripoles/#1/port width}\pgf@x
    \pgf@y=0pt\relax
}
\def\pgf@circ@amps@setup@letfedge#1{%
    \savedanchor\leftedge{%
        \pgf@circ@amps@leftedge{#1}
    }
    \anchor{leftedge}{\leftedge}
}
\def\pgf@circ@amps@setup@text#1{%
    \savedanchor{\textanchor}{%
        \ifpgf@circ@center@text
            \pgf@x=\dimexpr -.6\wd\pgfnodeparttextbox\relax
        \else
...
\pgfdeclareshape{op amp}
{
    ...
    \pgf@circ@amps@setup@letfedge{op amp}
    ...
    \anchor{north west}{ \northwest }
    \anchor{south east}{ \northwest \pgf@x=-\pgf@x \pgf@y=-\pgf@y }
    \pgf@circ@amps@setup@text{op amp}
    \pgf@circ@draw@component{
        \pgf@circ@setcolor
        \northwest
        \pgf@circ@res@up = \pgf@y
...
\pgfdeclareshape{en amp}
{
    ...
    \pgf@circ@amps@setup@letfedge{en amp}
    \anchor{rightedge}{\leftedge \pgf@x =-\pgf@x }
    \anchor{bin up}{
        \inOneFixed
        \pgf@ya=\pgf@y \leftedge \pgf@y=\pgf@ya
    ...
    \anchor{north west}{ \northwest }
    \anchor{south east}{ \northwest \pgf@x=-\pgf@x \pgf@y=-\pgf@y }
    \pgf@circ@amps@setup@text{en amp}
    \pgf@circ@draw@component{
        \pgf@circ@setcolor
        \northwest
        \pgf@circ@res@up = \pgf@y
...
\pgfdeclareshape{fd op amp}
{
    ...
    % support for border anchors
    \pgf@circ@amps@setup@letfedge{fd op amp}
    \anchor{rightedge}{\leftedge \pgf@x =-\pgf@x }
    \anchor{bin up}{
        \inOneFixed
        \pgf@ya=\pgf@y \leftedge \pgf@y=\pgf@ya
    ...
    \pgf@circ@amps@setup@text{fd op amp}
    \pgf@circ@draw@component{
        \pgf@circ@setcolor
...
\pgfdeclareshape{fd inst amp}
{
    ...
    % support for border anchors
    \pgf@circ@amps@setup@letfedge{fd inst amp}
    \anchor{rightedge}{\leftedge \pgf@x =-\pgf@x }
    \anchor{bin up}{
        \inOneFixed
        \pgf@ya=\pgf@y \leftedge \pgf@y=\pgf@ya
    ...
    \anchor{north west}{ \northwest }
    \anchor{south east}{ \northwest \pgf@x=-\pgf@x \pgf@y=-\pgf@y }
    \pgf@circ@amps@setup@text{fd inst amp}
...
\pgfdeclareshape{gm amp}
{
    ...
    % support for border anchors
    \pgf@circ@amps@setup@letfedge{gm amp}
    \anchor{rightedge}{\leftedge \pgf@x =-\pgf@x }
    \anchor{bin up}{
        \inOneFixed
        \pgf@ya=\pgf@y \leftedge \pgf@y=\pgf@ya
    ...
    \anchor{north west}{ \northwest }
    \anchor{south east}{ \northwest \pgf@x=-\pgf@x \pgf@y=-\pgf@y }
    \pgf@circ@amps@setup@text{gm amp}
    \pgf@circ@draw@component{
        \pgf@circ@setcolor
...
\pgfdeclareshape{inst amp}
{
    ...
    % support for border anchors
    \pgf@circ@amps@setup@letfedge{inst amp}
    \anchor{rightedge}{\leftedge \pgf@x =-\pgf@x }
    \anchor{bin up}{
        \inOneFixed
        \pgf@ya=\pgf@y \leftedge \pgf@y=\pgf@ya
    ...
    \anchor{north west}{ \northwest }
    \anchor{south east}{ \northwest \pgf@x=-\pgf@x \pgf@y=-\pgf@y }
    \pgf@circ@amps@setup@text{inst amp}
    % let's start drawing the component
    \pgf@circ@draw@component{
        \pgf@circ@setcolor
        %
...
\pgfdeclareshape{inst amp ra}
{
    ...
    % support for border anchors
    \pgf@circ@amps@setup@letfedge{inst amp ra}
    \anchor{rightedge}{\leftedge \pgf@x =-\pgf@x }
    \anchor{bin up}{
        \inOneFixed
        \pgf@ya=\pgf@y \leftedge \pgf@y=\pgf@ya
    ...
    \anchor{north west}{ \northwest }
    \anchor{south east}{ \northwest \pgf@x=-\pgf@x \pgf@y=-\pgf@y }
    \pgf@circ@amps@setup@text{inst amp ra}
...
\pgfdeclareshape{plain amp}
{
    ...
    % support for border anchors
    \pgf@circ@amps@setup@letfedge{plain amp}
    \anchor{rightedge}{\leftedge \pgf@x =-\pgf@x }
    \anchor{bin up}{
        \inOneFixed
        \pgf@ya=\pgf@y \leftedge \pgf@y=\pgf@ya
    ...
    \pgf@circ@amps@setup@text{plain amp}
    \pgf@circ@draw@component{
        \pgf@circ@setcolor
...
\pgfdeclareshape{plain mono amp}
{
    ...
    % support for border anchors
    \pgf@circ@amps@setup@letfedge{plain amp}% Warning: plain mono amp using plain amp settings
    \anchor{leftedge}{\leftedge}
    \anchor{rightedge}{\leftedge \pgf@x =-\pgf@x }
    \anchor{bin}{
        \inOne
    ...
    \pgf@circ@amps@setup@text{plain amp}% Warning: plain mono amp using plain amp settings
    \savedanchor{\textanchor}{%
        \ifpgf@circ@center@text
            \pgf@x=\dimexpr -.6\wd\pgfnodeparttextbox\relax
        \else
            \pgf@circ@amps@leftedge{plain amp}% Warning: plain mono amp using plain amp settings
            \pgfmathsetlength{\pgf@x}{\pgf@x + \ctikzvalof{left text distance}}
        \fi
        \pgf@y=\dimexpr.5\dp\pgfnodeparttextbox-.5\ht\pgfnodeparttextbox\relax
    }
...
% plain gm mono amplifier, no symbols, one input
\pgfdeclareshape{plain gm mono amp}
{
    \savedmacro{\ctikzclass}{\edef\ctikzclass{amplifiers}}
    \saveddimen{\scaledRlen}{\pgfmathsetlength{\pgf@x}{\ctikzvalof{\ctikzclass/scale}\pgf@circ@Rlen}}
    \anchor{center}{\pgfpointorigin}
    ...
    \pgf@circ@amps@setup@letfedge{gm amp}% Warning plain gm mono amp using gm amp settings
    \anchor{rightedge}{\leftedge \pgf@x =-\pgf@x }
    \anchor{bin}{
        \inOne
        \pgf@ya=\pgf@y \leftedge \pgf@y=\pgf@ya
    ...
    \pgf@circ@amps@setup@text{gm amp}% Warning plain gm mono amp using gm amp settings
    \pgf@circ@draw@component{
        \pgf@circ@setcolor
        ...
        \pgfscope
            ...
            \pgfpathmoveto{\pgfpoint{\ctikzvalof{tripoles/gm amp/port width}\pgf@circ@res@right@double}{0}}
            \pgfpathlineto{\pgfpoint{\ctikzvalof{tripoles/gm amp/port width}\pgf@circ@res@right@double}{\ctikzvalof{tripoles/gm amp/height 2}\pgf@circ@res@up}}
            \pgfpathlineto{\pgfpoint{0pt}{\pgf@circ@res@up}}
            \pgfpathlineto{\pgfpoint{0pt}{\pgf@circ@res@down}}
            \pgfpathlineto{\pgfpoint{\ctikzvalof{tripoles/gm amp/port width}\pgf@circ@res@right@double}{\ctikzvalof{tripoles/gm amp/height 2}\pgf@circ@res@down}}
            \pgfpathclose
            \pgf@circ@draworfill
        \endpgfscope
        \pgfpathmoveto{\pgfpoint
