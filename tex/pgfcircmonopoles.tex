...
\pgfdeclareshape{vcc}{
    ...
    \savedanchor{\textanchor}{
        \pgfmathsetlength{\pgf@circ@scaled@Rlen}{\ctikzvalof{\ctikzclass/scale}\pgf@circ@Rlen}
        \pgf@circ@res@step=\ctikzvalof{monopoles/vcc/width}\pgf@circ@scaled@Rlen
        %% this moves simply extends the bounding box. Clunky, but cannot be removed for compatibility
        \pgfpathmoveto{\pgfpoint{-.5\wd\pgfnodeparttextbox}{2\pgf@circ@res@step+2\ht\pgfnodeparttextbox}}
        \pgfpathmoveto{\pgfpoint{.5\wd\pgfnodeparttextbox}{2\pgf@circ@res@step+2\ht\pgfnodeparttextbox}}\pgf@x=0pt
...
% suggested by the TeX-SX answer https://tex.stackexchange.com/a/757589/38080
\ctikzset{monopoles/msrstub/radius/.initial=0.25}% public, thanks to @cis suggestion!
%>>>
...
\pgfdeclareshape{tlinestub}{
    ...
    % corrected for stable text; probably should take into account depth
    \savedanchor{\textoffset}{%
        \pgfpoint{-.5\wd\pgfnodeparttextbox}
        {-.5\dp\pgfnodeparttextbox+.5\ht\pgfnodeparttextbox}%
    }
    \anchor{text}{\pgfextractx{\pgf@xa}{\textoffset}\pgfextracty{\pgf@ya}{\textoffset}%
        \northeast\pgf@x=0.8\pgf@x\advance\pgf@x by \pgf@xa
...
\pgfdeclareshape{waves}{
    ...
    \anchor{text}{\pgfextracty{\pgf@ya}{\textoffset}%
        \northeast\advance\pgf@y by \pgf@ya
    }
    \anchor{top}{\northeast\pgf@x=0pt}
    \anchor{right}{\northeast\pgf@y=0pt}
...
\pgfdeclareshape{harmonics}{
    ...
    % text in the same position as waves
    \savedanchor{\textoffset}{%
        \pgf@y=\dimexpr+.5\dp\pgfnodeparttextbox-.5\ht\pgfnodeparttextbox\relax
    }
    \anchor{text}{\pgfextracty{\pgf@ya}{\textoffset}%
...
\pgfdeclareshape{bareantenna}{
    ...
    % stabilize all the factors in the text anchor position
    \savedmacro{\labelxanchor}{\edef\labelxanchor{\ctikzvalof{monopoles/bareantenna/label/xanchor}}}
    \savedmacro{\labelyanchor}{\edef\labelyanchor{\ctikzvalof{monopoles/bareantenna/label/yanchor}}}
    \savedanchor{\textoffset}{%
        \pgf@y=\dimexpr+.5\dp\pgfnodeparttextbox-.5\ht\pgfnodeparttextbox\relax
    }
    \anchor{text}{\pgfextracty{\pgf@ya}{\textoffset}%
        \northeast
        \pgf@x=\labelxanchor\pgf@x
        \pgf@y=\dimexpr\labelyanchor\pgf@y+\pgf@ya\relax
    }
    \anchor{top}{\northeast\pgf@x=0pt}
...
\pgfdeclareshape{dinantenna}{
    ...
    \anchor{text}{\pgfextracty{\pgf@ya}{\textoffset}%
        \northeast
        \pgf@y=\dimexpr.5\pgf@y+\pgf@ya\relax
    }
    \anchor{top}{\northeast\pgf@x=0pt}
    \anchor{right}{\northeast\pgf@y=0.5\pgf@y}
    \anchor{left}{\northeast\pgf@x=-\pgf@x\pgf@y=0.5\pgf@y}
...
\pgfdeclareshape{bareTXantenna}{
    ...
    \anchor{text}{\pgfextractx{\pgf@xa}{\textoffset}\pgfextracty{\pgf@ya}{\textoffset}%
        \northeast
        \pgf@x=\dimexpr-\labelxanchor\pgf@x-\pgf@xa\relax
        \pgf@y=\dimexpr\labelyanchor\pgf@y+\pgf@ya\relax
    }
...
\pgfdeclareshape{dynode}{
    ...
    \anchor{text}{\pgfextracty{\pgf@ya}{\textoffset}%
        ...
        \pgf@y=\dimexpr0.5\pgf@y+\pgf@ya\relax
    }
    \anchor{top}{\northeast\pgf@x=0pt}
    \anchor{right}{\northeast\pgf@y=0.5\pgf@y}
    \anchor{left}{\northeast\pgf@x=-\pgf@x\pgf@y=0.5\pgf@y}
...
\pgfdeclareshape{msrstub}{
    ...
    \pgf@circ@draw@component{
        ...
        \pgfextractx{\pgf@circ@res@left}{\northwest}
        \pgfextracty{\pgf@circ@res@down}{\southeast}
        \pgfmathsetlength{\pgf@circ@res@step}{\ctikzvalof{monopoles/msrstub/radius}*\pgf@circ@res@up}
        \pgfstartlinewidth=\pgflinewidth
        \pgfscope
            \pgfpathmoveto{\pgfpointorigin}
            \pgf@circ@setlinewidth{bipoles}{\pgfstartlinewidth}
            \ifdim\pgf@circ@res@step>0pt
                % avoid drawing the smaller arc if the user selected 0!
                \pgfpathlineto{\pgfpoint{0pt}{\pgf@circ@res@step}}
                \pgfusepath{draw}
...
\pgfdeclareshape{msport}{
    ...
    \anchor{left}{\northwest\pgf@y=0pt\relax}
    \anchor{right}{\southeast\pgf@y=0pt\relax}
    % put the node text inside and centered
    \savedanchor{\textanchor}{
        \pgfpoint{-.5\wd\pgfnodeparttextbox}{
...
\pgfdeclareshape{antenna}{
    ...
    % stabilize all the factors in the text anchor position
    \savedmacro{\labelxanchor}{\edef\labelxanchor{\ctikzvalof{monopoles/antenna/label/xanchor}}}
    \savedmacro{\labelyanchor}{\edef\labelyanchor{\ctikzvalof{monopoles/antenna/label/yanchor}}}
    \anchor{text}{% stable, but this text label position is not taking into account the text size.
        \northeast
        \pgf@x=\labelxanchor\pgf@x
        \pgf@y=\labelyanchor\pgf@y
    }
    \pgf@circ@draw@component{
        \pgfstartlinewidth=\pgflinewidth
        \pgf@circ@scaled@Rlen=\scaledRlen
...
\pgfdeclareshape{txantenna}{
    ...
    % stabilize all the factors in the text anchor position
    \savedmacro{\labelxanchor}{\edef\labelxanchor{\ctikzvalof{monopoles/txantenna/label/xanchor}}}
    \savedmacro{\labelyanchor}{\edef\labelyanchor{\ctikzvalof{monopoles/txantenna/label/yanchor}}}
    \anchor{text}{% stable, but this text label position is not taking into account the text size.
        \northeast
        \pgf@x=\labelxanchor\pgf@x
        \pgf@y=\labelyanchor\pgf@y
...
\pgfdeclareshape{rxantenna}{
    ...
    \anchor{south west}{\northeast\pgf@circ@res@step=\pgf@x\pgf@y=-\pgf@y\pgf@x=-\pgf@x\pgf@circ@shift@antenna@xy{1}{4}}
    % stabilize all the factors in the text anchor position
    \savedmacro{\labelxanchor}{\edef\labelxanchor{\ctikzvalof{monopoles/rxantenna/label/xanchor}}}
    \savedmacro{\labelyanchor}{\edef\labelyanchor{\ctikzvalof{monopoles/rxantenna/label/yanchor}}}
    \anchor{text}{% stable, but this text label position is not taking into account the text size.
        \northeast
        \pgf@x=\labelxanchor\pgf@x
        \pgf@y=\labelyanchor\pgf@y
